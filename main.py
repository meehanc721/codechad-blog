...
def register():
    ...
    if register_user_form.validate_on_submit():
        ...
        user = User.query.filter_by(email=register_user_form.email.data).first()
        if user:
            flash("Email already exists, try logging in.")
            return redirect(url_for("login"))

        safer_password = generate_password_hash(register_user_form.password.data, method="pbkdf2:sha256", salt_length=8)
        user_to_add = User(
...
def login():
    ...
    if login_user_form.validate_on_submit():
        ...
        # If they are a user and the password matches the databases
    ...
    return render_template("login.html", form=login_user_form, logged_in=current_user.is_authenticated)
